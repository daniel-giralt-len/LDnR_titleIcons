...
const SlotWrapper = styled.div`
  height: ${({size}) => size}px;
  transform: translateY(${({size, index}) => -1 * 204 * index}px); // TODO: Replace 204 with size once Icon's height can be sized down
...
const Slot = ({size, type: goalIcon, position}) => {
  ...
  return (<SlotWrapper size={size} index={currentIcon}>
    {iconList.map((type, index) => <Icon position={position} size={size} type={type} key={index} />)}
  </SlotWrapper>)
}
